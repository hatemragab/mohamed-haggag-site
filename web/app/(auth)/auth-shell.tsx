    ...
    <div className="auth-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: "100vh" }}>
      <div style={{ display: "flex", flexDirection: "column", padding: "24px" }}>
        <Link href="/" style={{ display: "inline-flex", alignItems: "center", gap: "10px", alignSelf: "flex-start" }}>
          ...
          <span style={{ display: "inline-flex", alignItems: "center", gap: "6px", color: "var(--ink-2)", fontSize: "14px", fontWeight: 700 }}><Icon name="chevR" size={15} /> {t.auth.backHome}</span>
        </Link>
      ...
      <div className="auth-aside" style={{ background: "linear-gradient(160deg,var(--navy-700),var(--navy-900))", position: "relative", overflow: "hidden", display: "grid", placeItems: "center", padding: "56px" }}>
        ...
        <div style={{ position: "relative", color: "#fff", maxWidth: "440px", textAlign: "center" }}>
          ...
          <h2 style={{ color: "#fff", fontSize: "26px", marginBottom: "14px", lineHeight: 1.4 }}>{mode === "register" ? t.auth.asideTitleRegister : t.auth.asideTitleLogin}</h2>
          <p style={{ color: "#c5d2dd", fontSize: "16px", lineHeight: 1.85 }}>{mode === "register" ? t.auth.asideTextRegister : t.auth.asideTextLogin}</p>
          <div style={{ display: "flex", gap: "14px", justifyContent: "center", marginTop: "34px", flexWrap: "wrap" }}>
            {([[t.auth.statStudentsValue, t.auth.statStudentsLabel], [t.auth.statLessonsValue, t.auth.statLessonsLabel], [t.auth.statSatisfactionValue, t.auth.statSatisfactionLabel]] as const).map(([v, l]) => (
