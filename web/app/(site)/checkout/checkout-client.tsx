...
const METHODS: { id: Method; labelKey: "methodCard" | "methodWallet" | "methodBank"; ic: IconName }[] = [
  { id: "card", labelKey: "methodCard", ic: "card" },
  { id: "wallet", labelKey: "methodWallet", ic: "phone" },
  { id: "bank", labelKey: "methodBank", ic: "shield" },
];
...
export function CheckoutClient({
  ...
  if (!plan) {
    return (
      <div className="wrap" style={{ padding: "80px 24px", textAlign: "center", maxWidth: "520px" }}>
        ...
        <h1 style={{ fontSize: "28px", marginBottom: "12px" }}>{t.checkout.noPlansTitle}</h1>
        <p style={{ color: "var(--ink-2)", fontSize: "16px", marginBottom: "28px" }}>{t.checkout.noPlansDesc}</p>
        <div style={{ display: "flex", gap: "12px", justifyContent: "center" }}>
          <Btn variant="primary" size="lg" onClick={() => router.push("/pricing")}>{t.checkout.backToPlans}</Btn>
        </div>
      </div>
    );
  }
  ...
  const curSymbol = t.common.currencies[cur.code].symbol;
  ...
  if (!user) {
    return (
      <div className="wrap" style={{ padding: "80px 24px", textAlign: "center", maxWidth: "520px" }}>
        ...
        <h1 style={{ fontSize: "28px", marginBottom: "12px" }}>{t.checkout.signInTitle}</h1>
        <p style={{ color: "var(--ink-2)", fontSize: "16px", marginBottom: "28px" }}>{t.checkout.signInDesc}</p>
        <div style={{ display: "flex", gap: "12px", justifyContent: "center" }}>
          <Btn variant="gold" size="lg" onClick={() => router.push("/register")}>{t.checkout.createAccount}</Btn>
          <Btn variant="outline" size="lg" onClick={() => router.push("/login")}>{t.checkout.signIn}</Btn>
        </div>
      </div>
    );
  }
  ...
  const pay = async () => {
    ...
    try {
      ...
      setPayError(e instanceof ApiError ? e.message : t.common.errorGeneric);
      setStage("form");
    }
  };
    ...
    return (
      <div className="wrap" style={{ padding: "70px 24px", maxWidth: "560px", textAlign: "center" }}>
        <div style={{ animation: "scaleIn .5s both" }}>
          ...
          <h1 style={{ fontSize: "32px", marginBottom: "14px" }}>{t.checkout.successHeading}</h1>
          <p style={{ color: "var(--ink-2)", fontSize: "17px", lineHeight: 1.8, marginBottom: "10px" }}>
            {t.checkout.successActivatedPrefix}
            {cat
              ? `${t.checkout.successActivatedTrack}${cat.title}${t.checkout.successActivatedTrackClose}`
              : `${t.checkout.successActivatedPlan}${plan.name}${t.checkout.successActivatedPlanClose}`}
            {t.checkout.successActivatedSuffix}
          </p>
          <p style={{ color: "var(--muted)", fontSize: "14px", marginBottom: "32px" }}>{t.checkout.successWatchHint}</p>
          <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
            <Btn variant="gold" size="lg" iconAfter="arrow" onClick={() => router.push(cat ? `/courses/${cat.slug}` : "/courses")}>{t.checkout.startWatching}</Btn>
            <Btn variant="outline" size="lg" icon="grid" onClick={() => router.push("/dashboard")}>{t.checkout.studentDashboard}</Btn>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ background: "var(--cream)", minHeight: "calc(100vh - 76px)" }}>
      <div className="wrap" style={{ padding: "40px 24px 80px" }}>
        <Link href="/pricing" style={{ display: "inline-flex", alignItems: "center", gap: "7px", color: "var(--muted)", fontSize: "14px", fontWeight: 700, marginBottom: "24px" }}><Icon name="chevR" size={16} /> {t.checkout.backToPlans}</Link>
        <div className="checkout-grid" style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: "32px", alignItems: "start" }}>
          ...
          <div style={{ background: "var(--paper)", borderRadius: "var(--r-lg)", border: "1px solid var(--line)", padding: "34px", boxShadow: "var(--shadow-sm)" }}>
            <h1 style={{ fontSize: "26px", marginBottom: "6px" }}>{t.checkout.checkoutTitle}</h1>
            <p style={{ color: "var(--ink-2)", fontSize: "15px", marginBottom: "26px" }}>{t.checkout.checkoutSubtitle}</p>
            ...
            <div style={{ marginBottom: "24px" }}>
              <div style={{ fontSize: "14px", fontWeight: 700, marginBottom: "10px" }}>{t.checkout.currencyLabel}</div>
              <div style={{ display: "flex", gap: "8px" }}>
                {CURRENCIES.map((c) => (
                  <button key={c.code} onClick={() => setCur(c)} style={{ padding: "10px 18px", borderRadius: "10px", fontSize: "14px", fontWeight: 700, background: cur.code === c.code ? "var(--navy-800)" : "var(--cream-2)", color: cur.code === c.code ? "#fff" : "var(--ink-2)" }}>{t.common.currencies[c.code].name}</button>
                ))}
              </div>
            </div>
            ...
            {/* طريقة الدفع */}
            <div style={{ marginBottom: "24px" }}>
              <div style={{ fontSize: "14px", fontWeight: 700, marginBottom: "10px" }}>{t.checkout.methodLabel}</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px" }}>
                {METHODS.map(({ id, labelKey, ic }) => (
                  <button key={id} onClick={() => setMethod(id)} style={{ padding: "16px 12px", borderRadius: "12px", border: `1.6px solid ${method === id ? "var(--gold)" : "var(--line)"}`, background: method === id ? "var(--gold-100)" : "var(--cream)", display: "flex", flexDirection: "column", alignItems: "center", gap: "8px", transition: "all .2s" }}>
                    ...
                    <span style={{ fontSize: "13px", fontWeight: 700, color: method === id ? "var(--navy-900)" : "var(--ink-2)" }}>{t.checkout[labelKey]}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {method === "card" && (
              <form onSubmit={(e) => { e.preventDefault(); void pay(); }}>
                <Field label={t.checkout.cardNumberLabel} icon="card" value={card.num} onChange={(e) => setCard({ ...card, num: e.target.value })} placeholder={t.checkout.cardNumberPlaceholder} />
                <Field label={t.checkout.cardNameLabel} icon="user" value={card.name} onChange={(e) => setCard({ ...card, name: e.target.value })} placeholder="MOHAMMED HAGGAG" />
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                  <Field label={t.checkout.cardExpiryLabel} value={card.exp} onChange={(e) => setCard({ ...card, exp: e.target.value })} placeholder="MM / YY" />
                  <Field label="CVC" type="password" value={card.cvc} onChange={(e) => setCard({ ...card, cvc: e.target.value })} placeholder="•••" />
                </div>
                <Btn variant="gold" size="lg" full type="submit" icon="lock" style={{ marginTop: "8px" }}>{t.checkout.payButtonPrefix}{num(price)} {curSymbol}{t.checkout.payButtonSuffix}</Btn>
              </form>
            )}
            {method !== "card" && (
              <div style={{ textAlign: "center", padding: "30px 20px", background: "var(--cream)", borderRadius: "var(--r)", border: "1px dashed var(--line)" }}>
                ...
                <p style={{ color: "var(--ink-2)", fontSize: "15px", marginBottom: "20px" }}>{method === "wallet" ? t.checkout.walletRedirect : t.checkout.bankRedirect}</p>
                <Btn variant="gold" size="lg" full icon="lock" onClick={() => void pay()}>{t.checkout.continuePayment}</Btn>
              </div>
            )}
            {stage === "processing" && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(10,31,54,.6)", backdropFilter: "blur(4px)", display: "grid", placeItems: "center", zIndex: 99 }}>
                <div style={{ background: "#fff", borderRadius: "var(--r-lg)", padding: "44px 56px", textAlign: "center", boxShadow: "var(--shadow-lg)" }}>
                  ...
                  <div style={{ fontWeight: 800, fontSize: "18px", color: "var(--navy-900)" }}>{t.checkout.processing}</div>
                  <div style={{ color: "var(--muted)", fontSize: "14px", marginTop: "6px" }}>{t.checkout.processingHint}</div>
                </div>
              </div>
            )}
            <div style={{ display: "flex", alignItems: "center", gap: "8px", justifyContent: "center", marginTop: "20px", color: "var(--muted)", fontSize: "13px" }}>
              <Icon name="lock" size={15} /> {t.checkout.secureNote}
            </div>
          </div>
          ...
          <div style={{ background: "var(--paper)", borderRadius: "var(--r-lg)", border: "1px solid var(--line)", padding: "30px", boxShadow: "var(--shadow-sm)", position: "sticky", top: "92px" }}>
            <h3 style={{ fontSize: "19px", marginBottom: "20px" }}>{t.checkout.orderSummary}</h3>
            <div style={{ display: "flex", gap: "14px", paddingBottom: "20px", borderBottom: "1px solid var(--line-2)", marginBottom: "20px" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: "13.5px", color: "var(--muted)", marginTop: "2px" }}>{cat ? t.checkout.fullTrackAccess : plan.period}</div>
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "12px", fontSize: "15px", marginBottom: "20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", color: "var(--ink-2)" }}><span>{cat ? t.checkout.fullTrackAccess : plan.period}</span><span>{num(price)} {curSymbol}</span></div>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "18px", borderTop: "2px solid var(--line)" }}>
              <span style={{ fontWeight: 800, fontSize: "17px", color: "var(--navy-900)" }}>{t.checkout.total}</span>
              <span style={{ fontWeight: 900, fontSize: "26px", color: "var(--navy-900)" }}>{num(price)} <span style={{ fontSize: "16px", color: "var(--gold-700)" }}>{curSymbol}</span></span>
            </div>
            <div style={{ marginTop: "22px", padding: "16px", background: "var(--cream)", borderRadius: "var(--r)", display: "flex", gap: "10px", alignItems: "flex-start" }}>
              ...
              <span style={{ fontSize: "13px", color: "var(--ink-2)", lineHeight: 1.6 }}>{t.checkout.refundGuarantee}</span>
            </div>
          </div>
        </div>
      </div>
