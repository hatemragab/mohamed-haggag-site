...
function Hero({ hero }: { hero: SiteContent["hero"] }) {
  ...
  return (
    <section style={{ position: "relative", overflow: "hidden", background: "linear-gradient(160deg,#fdfaf4 0%,#f5eede 100%)" }}>
      {/* زخارف خلفية */}
      <div style={{ position: "absolute", inset: 0, opacity: 0.5, backgroundImage: "radial-gradient(circle at 85% 15%, var(--gold-100), transparent 40%)", pointerEvents: "none" }} />
      ...
      <div className="wrap hero-grid" style={{ display: "grid", gridTemplateColumns: "1.05fr .95fr", gap: "48px", alignItems: "center", padding: "64px 24px 80px", position: "relative" }}>
        ...
        <div className="reveal" style={{ maxWidth: "600px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "10px", background: "rgba(255,255,255,.7)", border: "1px solid var(--line)", padding: "8px 16px", borderRadius: "999px", marginBottom: "26px", boxShadow: "var(--shadow-sm)" }}>
            ...
            <span style={{ fontSize: "13.5px", fontWeight: 700, color: "var(--navy-800)" }}>{t.home.heroBadge}</span>
          </div>
        ...
        <div className="reveal hero-photo" style={{ position: "relative", animationDelay: "120ms" }}>
          ...
          <div style={{ position: "relative", borderRadius: "var(--r-xl)", overflow: "hidden", boxShadow: "var(--shadow-lg)", border: "5px solid #fff", aspectRatio: "696/980", background: "var(--navy-800)" }}>
            <img src="/instructor.png" alt={t.home.instructorAlt} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top" }} />
            ...
            <div style={{ position: "absolute", bottom: "22px", insetInlineStart: "22px", insetInlineEnd: "22px", color: "#fff" }}>
              <div style={{ fontSize: "22px", fontWeight: 800 }}>{t.home.instructorName}</div>
              <div style={{ fontSize: "14px", color: "var(--gold-200)", fontWeight: 600 }}>{t.home.instructorTitle}</div>
            </div>
          </div>
          {/* بطاقة عائمة */}
          <div style={{ position: "absolute", top: "30px", insetInlineEnd: "-22px", background: "#fff", borderRadius: "var(--r)", padding: "14px 18px", boxShadow: "var(--shadow-lg)", display: "flex", alignItems: "center", gap: "12px", animation: "fadeUp .8s .5s both" }} className="float-card">
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: "15px", color: "var(--navy-900)" }}>{t.home.floatCardTitle}</div>
...
function StatsStrip({ stats }: { stats: SiteContent["instructor"]["stats"] }) {
    ...
    <section style={{ background: "var(--navy-900)", position: "relative", overflow: "hidden" }}>
      ...
      <div className="wrap" style={{ padding: "52px 24px" }}>
        <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "24px" }}>
          {stats.map((s, i) => (
            <div key={i} style={{ textAlign: "center", borderInlineEnd: i < stats.length - 1 ? "1px solid rgba(255,255,255,.1)" : "none" }} className="stat-cell">
...
function WhySection({ why }: { why: SiteContent["why"] }) {
  const { t } = useLocale();
...
function LearnSection({
  ...
  learnSection: SiteContent["learnSection"];
  learn: string[];
}) {
  const { t } = useLocale();
  return (
    <section className="section" style={{ background: "linear-gradient(180deg,var(--cream),var(--cream-2))" }}>
      <div className="wrap learn-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1.1fr", gap: "56px", alignItems: "center" }}>
        <div>
          <span className="eyebrow">{t.home.learnEyebrow}</span>
          <h2 style={{ fontSize: "clamp(28px,3.5vw,40px)", margin: "16px 0 18px" }}>{learnSection.title}</h2>
        ...
        <div style={{ position: "relative" }}>
          <div style={{ background: "var(--navy-900)", borderRadius: "var(--r-xl)", padding: "40px", color: "#fff", boxShadow: "var(--shadow-lg)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "14px", borderTop: "1px solid rgba(255,255,255,.12)", paddingTop: "22px" }}>
              <img src="/instructor.png" alt="" style={{ width: 54, height: 54, borderRadius: "50%", objectFit: "cover", objectPosition: "center 20%", border: "2px solid var(--gold-400)" }} />
              <div>
                <div style={{ fontWeight: 800, fontSize: "16px" }}>{t.home.learnAuthorName}</div>
                <div style={{ fontSize: "13px", color: "var(--gold-200)" }}>{t.home.learnAuthorRole}</div>
              </div>
            </div>
          </div>
        </div>
...
function CoursesPreview({ categories }: { categories: CategoryListItem[] }) {
  const router = useRouter();
  const { t } = useLocale();
  return (
      ...
      <div className="wrap">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "44px", flexWrap: "wrap", gap: "16px" }}>
          <div style={{ maxWidth: "560px" }}>
            ...
            <h2 style={{ fontSize: "clamp(28px,3.8vw,40px)", margin: "14px 0 12px" }}>{t.home.coursesTitle}</h2>
            <p style={{ color: "var(--ink-2)", fontSize: "17px" }}>{t.home.coursesSub}</p>
          </div>
          <Btn variant="outline" iconAfter="arrow" onClick={() => router.push("/courses")}>{t.home.coursesViewAll}</Btn>
        </div>
        {categories.length === 0 ? (
          <div style={{ textAlign: "center", padding: "56px 24px", background: "var(--paper)", border: "1px dashed var(--line)", borderRadius: "var(--r-lg)", color: "var(--muted)", fontWeight: 600 }}>
            {t.home.coursesEmpty}
          </div>
        ) : (
          <div className="cat-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "24px" }}>
            {categories.map((c, i) => <CategoryCard key={c.id} cat={c} idx={i} />)}
          </div>
        )}
      </div>
    </section>
...
function AccessSection({ steps }: { steps: SiteContent["accessSteps"] }) {
  const { t, num } = useLocale();
  ...
  return (
    <section className="section" style={{ background: "var(--cream-2)" }}>
      <div className="wrap">
        <div className="section-head">
          <span className="eyebrow">{t.home.accessEyebrow}</span>
            ...
            <div key={s.n} className="reveal" style={{ position: "relative", animationDelay: `${i * 80}ms` }}>
              <div style={{ background: "var(--paper)", borderRadius: "var(--r-lg)", padding: "30px 26px", border: "1px solid var(--line)", boxShadow: "var(--shadow-sm)", height: "100%" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "16px" }}>
                  <span style={{ width: "46px", height: "46px", borderRadius: "13px", background: "linear-gradient(150deg,var(--navy-600),var(--navy-900))", color: "var(--gold-400)", display: "grid", placeItems: "center", fontWeight: 900, fontSize: "20px" }}>{num(s.n)}</span>
                  {i < steps.length - 1 && <span style={{ flexGrow: 1, height: "2px", background: "repeating-linear-gradient(to left,var(--gold-200) 0 6px,transparent 6px 12px)" }} className="step-line" />}
                </div>
      ...
      <div className="wrap">
        ...
        <div className="testi-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "22px" }}>
          {testimonials.map((t, i) => (
            <div key={t.id} className="reveal" style={{ background: "var(--paper)", borderRadius: "var(--r-lg)", padding: "28px", border: "1px solid var(--line)", boxShadow: "var(--shadow-sm)", animationDelay: `${i * 60}ms` }}>
...
function FaqPreview({ faq }: { faq: SiteContent["faq"] }) {
  const router = useRouter();
  const { t } = useLocale();
    ...
    <section className="section">
      <div className="wrap" style={{ maxWidth: "860px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {faq.slice(0, 5).map((f, i) => (
            <div key={i} style={{ background: "var(--paper)", borderRadius: "var(--r)", border: "1px solid var(--line)", overflow: "hidden", boxShadow: open === i ? "var(--shadow)" : "var(--shadow-sm)", transition: "box-shadow .25s" }}>
              <button onClick={() => setOpen(open === i ? -1 : i)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", padding: "20px 24px", textAlign: "start" }}>
        ...
        <div style={{ textAlign: "center", marginTop: "28px" }}>
          <Btn variant="ghost" iconAfter="arrow" onClick={() => router.push("/faq")}>{t.home.faqViewAll}</Btn>
        </div>
      </div>
    </section>
  );
...
export function HomeClient({
  ...
  return (
    <>
      ...
      <CoursesPreview categories={sortedCategories} />
      <AccessSection steps={content.accessSteps} />
      <Testimonials testimonials={topTestimonials} />
      <FaqPreview faq={content.faq} />
      <FinalCTA />
    </>
  );
}
